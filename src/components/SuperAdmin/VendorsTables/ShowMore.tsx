...
const VendorDetails: React.FC = () => {
  ...
  if (loading) {
    ...
    const axiosError = vendorError as AxiosError;
    pageStatus = PageStatus.ERROR;
    if (
      axiosError?.response?.status === 401 ||
      axiosError?.response?.status === 403
    ) {
      pageError = t("VendorsTable:vendorsPage.errors.global");
    } else {
      pageError = t("VendorsTable:vendorsPage.details.generalError");
    }
  } else if (!vendor) {
  ...
  return (
    <PageStatusHandler
      status={pageStatus}
      loadingText={t("vendorsPage.details.loading")}
        ...
        <div className="vendor-details p-6 max-w-5xl mx-auto space-y-8 text-gray-800 dark:text-gray-100">
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <p>
                <strong>{t("vendorsPage.details.email")}:</strong>{" "}
                {vendor?.email}
              </p>
              <p>
                <strong>{t("vendorsPage.details.phone")}:</strong>{" "}
                {vendor?.phone}
              </p>
              <p>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
            ...
            <div className="flex justify-center items-center mt-6">
              <button
                ...
                className="block px-5 py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-lg shadow  transition"
              >
