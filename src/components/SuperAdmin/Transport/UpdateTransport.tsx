...
import {  useGetTransportationPriceById, useUpdateransportationPrice } from "../../../hooks/Api/SuperAdmin/useTransports/useSuperAdminTransports";
import Select from "../../common/form/Select";
import Checkbox from "../../common/input/Checkbox";

const UpdateTransport = () => {
  const { t } = useTranslation(["UpdateTransport"]);
  const { id } = useParams();
  ...
  const [transportData, setTransportData] = useState<TransportInputData>({
    ...
    const { data:transport }=useGetTransportationPriceById(id);
    
      useEffect(() => {
        if (!transport) return;
        setTransportData({
          area_ar: transport.area_ar,
          area_en: transport.area_en,
          is_free: transport.is_free === 0 ? false : true,
          vehicle_type: transport.vehicle_type,
          fixed_price: transport.fixed_price.toString(),
        });
      }, [transport]);
  ...
  const validate = () => {
    ...
    if (!transportData.area_ar) newErrors.area_ar = t("transport.errors.area");
    if (!transportData.area_en) newErrors.area_en = t("transport.errors.area");
    if (!transportData.vehicle_type) newErrors.vehicle_type = t("transport.errors.vehicle_type");
  ...
  const { mutateAsync: updateTransport } = useUpdateransportationPrice(id!);
  const handleChange = (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isCurrentlyOnline()) {
      toast.error(t("transport.errors.no_internet"));
      return;
    }
    ...
    try {
      await updateTransport({ transportData: formData, id:id! });
  ...
  return (
    <div>
      <SEO
        title={{
          ar: " تعديل سعر نقل جديد (سوبر أدمن)",
          en: "Create New transport (Super Admin)",
        }}
        description={{
          ar: "صفحة تعديل سعر نقل  بواسطة المشرف العام في تشطيبة. أدخل التفاصيل،.",
      ...
      <form onSubmit={handleSubmit} className="space-y-6 mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <Label htmlFor="area_ar">{t("transport.area_ar")}</Label>
            <Input
              name="area_ar"
          ...
          <div>
            <Label htmlFor="area_en">{t("transport.area_en")}</Label>
            <Input
            name="area_en"
            ...
            />
            {errors.area_en[0] && (
              <p className="text-red-500 text-sm mt-1">{errors.area_en[0]}</p>
            )}
