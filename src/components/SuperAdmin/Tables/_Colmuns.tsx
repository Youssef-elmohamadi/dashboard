...
export const buildColumns = <T extends BaseEntity>(
  ...
  if (options.includeFixedPrice) {
    columns.push({
      Header: t("table.fixed_price"),
      accessor: "fixed_price" as keyof T,
    });
  }
  if (options.includeLine) {
    columns.push({
      Header: t("table.line"),
      accessor: "line" as keyof T,
    });
  }
  if (options.includeArea) {
    columns.push({
      Header: t("table.area"),
      accessor: `area_${lang}` as keyof T,
    });
  }
  if (options.includeVehicleType) {
    columns.push({
      Header: t("table.vehicle_type"),
