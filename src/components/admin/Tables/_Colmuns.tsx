...
export const buildColumns = <T extends BaseEntity>(
  ...
  if (options.includeTitle) {
    columns.push({
      Header: t("table.question_title"),
      accessor: `title_${lang}` as keyof T,
    });
  }
  if (options.includeQuestion) {
    columns.push({
      Header: t("table.question"),
      accessor: `description_${lang}` as keyof T,
    });
  }

  if (options.includeQuestionType) {
    columns.push({
      Header: t("table.question_type"),
      accessor: "type" as keyof T,
      Cell: ({ value }: { value: string }) => {
        if (value === "single_choice") {
          return t("table.single_choice");
        }
        if (value === "multiple_choice") {
          return t("table.multi_choice");
        }
        return value;
      },
    });
